...
def setup_keys():
    if os.path.exists("keys.cfg") == False:
        sys.exit("The 'keys.cfg' file wasn't found !")
	#read File
...
def pkg(tid):
    if  os.path.exists("pkg_dec.exe") == False: 
        sys.exit("The pkg_dec.exe wasn't found !")
    else:
        i = search_tid(tid)
        link = read_cell(3, i)
        download(link, 'temp.pkg')
        zrif = read_cell(4, i)
